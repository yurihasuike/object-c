...
NSString* const ProfileLayoutCellKind      = @"ProfileCell";
NSString* const ProfileLayouDecorationKind = @"ProfileDecorationCell";

@interface ProfileCollectionViewLayout()
...
@implementation ProfileCollectionViewLayout
...
- (void)setup {
    [self registerClass:[ProfileDecorationCollectionReusableView class] forDecorationViewOfKind:ProfileLayouDecorationKind];
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (UICollectionViewLayoutAttributes *layoutAttributes in allAttributes) {
        if ([layoutAttributes.representedElementKind isEqualToString:UICollectionElementKindSectionHeader]) {
            ...
            origin.y = MIN(
                           MAX(self.collectionView.contentOffset.y,
                               (CGRectGetMinY(firstCellAttrs.frame) - headerHeight) - self.topInset),
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    return self.layoutInfo[ProfileLayoutCellKind][indexPath];
}
...
(NSString*)decorationViewKind atIndexPath:(NSIndexPath *)indexPath {
    return self.layoutInfo[ProfileLayouDecorationKind][indexPath];
}
...
- (void) calculateItemsAttributes {
    ...
    [UICollectionViewLayoutAttributes layoutAttributesForDecorationViewOfKind:ProfileLayouDecorationKind
    ...
    newLayoutInfo[ProfileLayoutCellKind] = cellLayoutInfo;
    newLayoutInfo[UICollectionElementKindSectionHeader] = titleLayoutInfo;
    newLayoutInfo[ProfileLayouDecorationKind] = @{indexPath: emblemAttributes};
...
- (CGRect)frameForWaterfallCellIndexPath:(NSIndexPath *)indexPath {
    ...
    for (NSInteger column = 0; column < self.columnsCount; column++) {
        ...
        // custom
        columnsHeights[column] = [self headerHeightForIndexPath:indexPath] - self.topInset + 4.0f;
    }
...
- (CGRect)frameForWaterfallHeaderAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return CGRectMake(0, 0, self.collectionView.bounds.size.width, height);
}

- (CGRect) frameForWaterfallDecoration {
    CGSize size = [ProfileDecorationCollectionReusableView defaultSize];
