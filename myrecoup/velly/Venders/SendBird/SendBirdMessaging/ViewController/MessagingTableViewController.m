...
//    if (viewMode == kMessagingChannelListViewMode || viewMode == kMessagingChannelListEditViewMode) {
...
{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"SendBird" message:INTEAGE_IN_APP_VER delegate:nil cancelButtonTitle:@"Close" otherButtonTitles:nil];
    [alert show];
}
...
- (void) startChatting
{
    ...
    [SendBird loginWithUserId:self.userId andUserName:self.userName andUserImageUrl:self.userImageUrl andAccessToken:@""];
...
- (void) applyConstraints
{
    ...
    // Message Input View
    [self.view addConstraint:[NSLayoutConstraint constraintWithItem:self.messageInputView
                                                          attribute:NSLayoutAttributeLeading
                                                          relatedBy:NSLayoutRelationEqual
                                                             toItem:self.view
                                                          attribute:NSLayoutAttributeLeading
    ...
    [self.view addConstraint:[NSLayoutConstraint constraintWithItem:self.messageInputView
                                                         ...
                                                         multiplier:1
                                                           constant:0]];
    bottomMargin = [NSLayoutConstraint constraintWithItem:self.messageInputView
                                               ...
                                               multiplier:1
                                                 constant:-50];
    if ([[UIScreen mainScreen] applicationFrame].size.width >= 414) {
        bottomMargin.constant = 0;
    }
    [self.view addConstraint:bottomMargin];
    [self.view addConstraint:[self getInputViewConstraint:NO]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == self.channelMemberListTableView) {
        ...
        if ([self shouldShowMessage]){
            //メッセージリストがなければx秒後に再度確認しメッセージを出す
            float x = 0.5;
            [CommonUtil delay:x block:^{
                if ([self shouldShowMessage]) {
                    [self showMessage:@"MsgNoChannel" to:self.view];
                }
            }];
        }else{
            [self dissmissViewFrom:self.view];
        }
        ...
        if (viewMode == kMessagingViewMode && !messageArray.count) {
            float x = 0.5;
            [CommonUtil delay:x block:^{
                if (viewMode == kMessagingViewMode && !messageArray.count) {
                    [self showMessage:@"MsgNoMessage" to:self.tableView];
                }
            }];
        }else{
            [self dissmissViewFrom:self.tableView];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == self.channelMemberListTableView) {
        ...
        if (viewMode == kMessagingChannelListEditViewMode) {
            ...
            viewMode = kMessagingViewMode;
            [self.view removeConstraint:[self getInputViewConstraint:NO]];
            [self.view addConstraint:[self getInputViewConstraint:YES]];
            [self setNavigationButton];
        }
    }
    else {
...
- (void) clickURL:(NSURL *)url
{
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:[url absoluteString]
                                               ...
                                               destructiveButtonTitle:nil
                                                    otherButtonTitles:NSLocalizedString(@"OpenLinkInSafari", nil), nil];
...
- (void) clickImage:(NSURL *)url
{
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:[url absoluteString]
                                                    ...
                                                    cancelButtonTitle:NSLocalizedString(@"Cancel", nil)
                                               destructiveButtonTitle:nil
                                                    otherButtonTitles:NSLocalizedString(@"OpenImageInSafari", nil), nil];
...
- (BOOL)shouldShowMessage {
    return ((viewMode == kMessagingChannelListViewMode ||
             viewMode == kMessagingChannelListEditViewMode) && !messagingChannels.count);
}
///メッセージを出す
- (void)showMessage:(NSString *)msg to:(UIView *)to {
    NetworkErrorView *noMessageView = [[NetworkErrorView alloc]init];
    noMessageView.delegate = self;
    [noMessageView setBackgroundColor:[UIColor whiteColor]];
    [noMessageView.noNetworkRetryBtn setHidden:YES];
    [noMessageView.noNetworkImageView setHidden:YES];
    [noMessageView.noNetworkLabel setText:msg.localize];
    [noMessageView showInView:to];
}
///メッセージリストなら0を返す
...
/**
 * メッセージ送信フォームの表示・非表示を決定.
 * @param BOOL 再定義するかどうか
 * @return NSLayoutConstraint 高さの定義
 */
- (NSLayoutConstraint *)getInputViewConstraint:(BOOL)reset {
    if (!self.inputViewContstraint || reset) {
        self.inputViewContstraint = [NSLayoutConstraint constraintWithItem:self.messageInputView
                                                                 attribute:NSLayoutAttributeHeight
                                                                 relatedBy:NSLayoutRelationEqual
                                                                    toItem:nil
                                                                 attribute:NSLayoutAttributeNotAnAttribute
                                                                multiplier:1
                                                                  constant:[self getInputViewHeight]];
    }
    return self.inputViewContstraint;
}

#pragma mark - NetworkErrorViewDelete
//「メッセージがありません」を消す
- (void)dissmissViewFrom:(UIView *)view
{
