...
//    UIView *pageView = [[UIView alloc] initWithFrame:CGRectMake(*xIndex, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height)];
    ...
    // custom
    UIView *pageView = [[UIView alloc] initWithFrame:CGRectMake(*xIndex + 20, 0 + 20, self.scrollView.frame.size.width - 40, self.scrollView.frame.size.height -40)];
    pageView.backgroundColor = [UIColor colorWithWhite:1.0f alpha:0.8f];
    ...
    *xIndex += self.scrollView.frame.size.width;
    ...
    if(page.titleIconView) {
        ...
        // custom
        rect1.origin.x = (pageView.bounds.size.width - rect1.size.width)/2;
        rect1.origin.y = page.titleIconPositionY - 20;
    ...
    if(page.pageIconView) {
        UIView *pageImageView = page.pageIconView;
        CGRect rect2 = pageImageView.frame;
        //rect1.origin.x = (self.scrollView.frame.size.width - rect1.size.width)/2;
        // custom
        rect2.origin.x = (pageView.bounds.size.width - rect2.size.width)/2;
        rect2.origin.y = page.pageIconPositionY - 20;
        pageImageView.frame = rect2;
        pageImageView.tag = kTitleImageViewTag;
...
#pragma clang diagnostic pop
        ...
        CGRect titleLabelFrame = CGRectMake(self.frame.size.width/3, self.frame.size.height - 70, self.scrollView.frame.size.width - 20, titleHeight);
    ...
    if([page.desc length]) {
        ...
        if(page.descWidth != 0) {
            descLabelFrame = CGRectMake((self.frame.size.width - page.descWidth)/2, self.frame.size.height/2 - page.descPositionY, page.descWidth, 500);
        } else {
            descLabelFrame = CGRectMake(10, self.frame.size.height/2 - page.descPositionY, self.scrollView.frame.size.width - 60, 80);
        }
    ...
    if([page.navimsg length]) {
        CGRect navimsgLabelFrame;
        
        if(page.navimsgWidth != 0) {
            navimsgLabelFrame = CGRectMake((self.frame.size.width - page.navimsgWidth)/2, self.frame.size.height/2 - page.navimsgPositionY, page.navimsgWidth, 500);
        } else {
            navimsgLabelFrame = CGRectMake(10, self.frame.size.height/2 - page.navimsgPositionY, self.scrollView.frame.size.width - 60, 80);
        }
        
        UITextView *navimsgLabel = [[UITextView alloc] initWithFrame:navimsgLabelFrame];
        navimsgLabel.text = page.navimsg;
        navimsgLabel.scrollEnabled = NO;
        navimsgLabel.font = page.navimsgFont;
        navimsgLabel.textColor = page.navimsgColor;
        navimsgLabel.backgroundColor = [UIColor clearColor];
        navimsgLabel.textAlignment = NSTextAlignmentCenter;
        navimsgLabel.userInteractionEnabled = NO;
        navimsgLabel.tag = kDescLabelTag;
        
        [pageView addSubview:navimsgLabel];
    }
    
    if([page.btnmsg length]){
        CGRect navimsgLabelFrame;
        
        if(page.navimsgWidth != 0) {
            navimsgLabelFrame = CGRectMake((self.frame.size.width - page.navimsgWidth)/2, self.frame.size.height/2 - page.navimsgPositionY - 26, page.navimsgWidth, 500);
        } else {
            navimsgLabelFrame = CGRectMake(10, self.frame.size.height/2 - page.navimsgPositionY - 26, self.scrollView.frame.size.width - 60, 80);
        }

        UIButton *btnmsgBtn = [[UIButton alloc] initWithFrame:navimsgLabelFrame];
        //btnmsgBtn.titleLabel.text = page.btnmsg;
        [btnmsgBtn setTitle:page.btnmsg forState:UIControlStateNormal];
        [btnmsgBtn.titleLabel setFont:page.navimsgFont];
        btnmsgBtn.tintColor = page.navimsgColor;
        [btnmsgBtn setTitleColor:page.navimsgColor forState:UIControlStateNormal];
        [btnmsgBtn setBackgroundColor:[UIColor clearColor]];
        btnmsgBtn.titleLabel.textAlignment = NSTextAlignmentCenter;
        btnmsgBtn.userInteractionEnabled = YES;
        [btnmsgBtn addTarget:self action:@selector(nextPageScroll:) forControlEvents:UIControlEventTouchUpInside];
        [pageView addSubview:btnmsgBtn];

    }
    
    if([page.btnclosemsg length]){
        CGRect navimsgLabelFrame;
        ...
        UIButton *btnclosemsgBtn = [[UIButton alloc] initWithFrame:navimsgLabelFrame];
        //btnmsgBtn.titleLabel.text = page.btnmsg;
        [btnclosemsgBtn setTitle:page.btnmsg forState:UIControlStateNormal];
        [btnclosemsgBtn.titleLabel setFont:page.navimsgFont];
        btnclosemsgBtn.tintColor = page.navimsgColor;
        [btnclosemsgBtn setTitleColor:page.navimsgColor forState:UIControlStateNormal];
        [btnclosemsgBtn setBackgroundColor:[UIColor clearColor]];
        btnclosemsgBtn.titleLabel.textAlignment = NSTextAlignmentCenter;
        btnclosemsgBtn.userInteractionEnabled = YES;
        [btnclosemsgBtn addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchUpInside];
        [pageView addSubview:btnclosemsgBtn];
...
- (void)nextPageScroll:(id)sender
{
    self.visiblePageIndex = (self.scrollView.contentOffset.x + self.scrollView.bounds.size.width/2)/self.scrollView.frame.size.width;
    if (self.visiblePageIndex < _pages.count) {
        self.visiblePageIndex++;
        self.pageControl.currentPage = self.visiblePageIndex;
        [self makePanelVisibleAtIndex:self.visiblePageIndex];
        CGFloat mainWidth = [[UIScreen mainScreen]bounds].size.width;
        [self.scrollView setContentOffset:CGPointMake( mainWidth * self.visiblePageIndex, 0) animated:YES];
    }
}
