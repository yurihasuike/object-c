...
- (instancetype)initWithFrame:(CGRect)frame tabViews:(NSArray *)tabViews tabBarHeight:(CGFloat)height tabColor:(UIColor *)color {
  ...
  if (self) {
    ...
    // custom
    //[contentView setBackgroundColor:HEADER_BG_COLOR];
    ...
    for (UIView *tab in tabViews) {
      ...
      // 幅の変更はココデ
      [VFL appendFormat:@"-%f-[T%d(%f)]", index ? 20.0f : 20.0 + widthDifference / 2, index, tab.frame.size.width];
      [views setObject:tab forKey:[NSString stringWithFormat:@"T%d", index]];
      
      [contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-2-[T]-2-|"
                                                                          options:0
                                                                          metrics:nil
                                                                            views:@{@"T": tab}]];
      [tab setTag:index];
    ...
    [VFL appendString:[NSString stringWithFormat:@"-%f-|", 20.0f + widthDifference / 2]];
    ...
    //[tabIndicator setBackgroundColor:HEADER_BG_COLOR];
...
- (void)animateToTabAtIndex:(NSInteger)index {
  ...
  // custom : current color update
  for (int j = 0; j < [[self tabViews]count]; j++) {
      UIView *testview = nil;
      UILabel *test = nil;
      testview = [self tabViews][j];
      if(j == index){
          if( [testview isKindOfClass:[UILabel class]] ){
              test = (UILabel *)testview;
              test.textColor = HEADER_BG_COLOR;
          }
      }else{
          if( [testview isKindOfClass:[UILabel class]] ){
              test = (UILabel *)testview;
